...
class InstallController extends Controller
{
	...
	public function siteInfo(Request $request)
	{
		...
		if ($request->isMethod('post')) {
            ...
            $filename = 'database/vip.sql';
            $filePath = storage_path($filename);

            // if(file_exists($filePath)) {
            //     // Save the new database file
            //     File::put($filePath, $purchase_code_data->schema);
            // }
	...
	public function databaseImport(Request $request)
	{
		...
		if ($request->action == 'import') {
			...
			return redirect($this->installUrl . '/finish');
		}
	...
	private function writeEnv($request)
	{
		...
		// Get app version
		$version = config('inilabs.iniconfig.item_version');
	...
	private function updateDatabase($pdo, $tablesPrefix, $siteInfo)
	{
		...
		try {
			...
			$sql = 'INSERT INTO `' . $tablesPrefix . 'users` (`id`, `name`, `email`, `phone`, `password`) VALUES (1, :name, :email, null, :password);';
			...
			$appSettings = [
				...
				'purchase_username'     => isset($siteInfo['purchase_username']) ? $siteInfo['purchase_username'] : '',
				'site_name'             => isset($siteInfo['site_name']) ? $siteInfo['site_name'] : '',
				'site_email'            => isset($siteInfo['email']) ? $siteInfo['email'] : '',
			];
            $sql = 'INSERT INTO `' . $tablesPrefix . 'settings` (`key`, `value`) VALUES (:key, :value);';
            $query = $pdo->prepare($sql);
    ...
    private function purchaseCodeChecker(Request $request)
    {
        $postData = array(
            ...
            'username'      => $request->purchase_username,
            'ip'            => Ip::get(),
            'domain'        => getDomain(),
            'purpose'       => 'install',
            'product_name'  => config('inilabs.iniconfig.item_name'),
            'version'       => 4.2,
            'email'         => $request->email,
        );
        $apiUrl = config('inilabs.iniconfig.purchaseCodeCheckerUrl');
